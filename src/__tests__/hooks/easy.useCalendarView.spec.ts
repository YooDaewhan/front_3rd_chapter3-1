...
jest.mock('../apis/fetchHolidays');

describe('useCalendarView', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    fetchHolidays.mockImplementation((date) => {
      const month = date.getMonth() + 1;
      const year = date.getFullYear();
      if (year === 2024 && month === 1) return { '2024-01-01': '신정' };
      if (year === 2024 && month === 10) return { '2024-10-03': '개천절', '2024-10-09': '한글날' };
      return {};
    });
  });
  ...
  it("currentDate가 '2024-01-01'로 변경되면 1월 휴일 '신정'으로 업데이트되어야 한다", () => {
    const { result } = renderHook(() => useCalendarView());
